    ...
    try:
        if os.path.exists(COOKIE_PATH):
            ...
            for i, cookie in enumerate(cookies, 1):
                ...
                if info:
                    print(f"[green]Cookie {i} is valid")
...
def generate_user_agent():
    ...
    return f"Mozilla/5.0 (Linux; Android {random.randint(4,13)}; {random.choice(amazon)}; Windows 10 Mobile) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(84,106)}.0.{random.randint(4200,4900)}.{random.randint(40,140)} Mobile Safari/537.36"

def get_lsd_token(session):
    try:
        git_fb = session.get("https://touch.facebook.com/pages/create/?ref_type=registration_form", timeout=10).text
...
def get_cookie(uid, pww, ua=None):
    try:
        ...
        response = session.post(url, data=_data, headers=_header, allow_redirects=False, timeout=10)
...
def bulk_cookie_getter():
    try:
        ...
        if not accounts:
            print(Panel("[red]No valid accounts found!", 
                title="[bright_white]>> [Error] <<",
                width=65,
                style="bold bright_white"
            ))
...
def update_tool():
    try:
        ...
        if "Already up to date" in result.stdout:
            print(Panel("[green]Tool is already up to date!", title="[bright_white]>> [Update Status] <<", width=65))
        else:
            print(Panel("[green]Tool updated successfully!", title="[bright_white]>> [Update Status] <<", width=65))
            sys.exit(0)
    except Exception as e:
        print(Panel(f"[red]Update failed: {str(e)}", title="[bright_white]>> [Error] <<", width=65))
...
class FacebookShare:
    ...
    def get_token(self):
        try:
            response = self.session.get('https://business.facebook.com/content_management', timeout=10)
    ...
    def share_post(self):
        ...
        while count < self.share_count:
            try:
                ...
                response = self.session.post('https://graph.facebook.com/v18.0/me/feed', data=post_data, timeout=10)
                data = response.json()
                ...
                print(f"Error sharing post with cookie {self.cookie_index + 1}")
                self.stats.update_failed(self.cookie_index)
                self.remove_cookie()
                break
